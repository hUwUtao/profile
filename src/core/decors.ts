...
function lazy(
  ...
  return new Promise<() => boolean>((ok, _panic) =>
    a.forEach((e) => {
...
async function invoke() {
  return await lazy([
    {
      cond:
        (month == 11 && date > 20) || month == 12 || (month == 1 && date < 20),
      draw: (frame) => ({
        ...parcf,
        origin: {
          x: Math.random(),
